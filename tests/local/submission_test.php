...
final class submission_test extends \basic_testcase {
     ...
     * string raises an "Array to string conversion" warning and renders the word "Array" as
    ...
    public function test_a_non_scalar_value_is_dropped(): void {
        ...
        /* The control: the well-formed entry beside it survives, so a reader that dropped the
           whole envelope fails here. */
     ...
     *
     * The snapshot formatter masks by matching each entry's key against the keys the reader may
     * see, so an entry with an unusable key cannot be masked correctly and is withheld rather
     * than repaired to an empty string.
     *
     * @return void
     */
     ...
     *
     * The guard above must not have narrowed what a normal snapshot yields. A number is included
     * because an envelope from another site's archive may carry one; this plugin writes strings,
     * and is_scalar() admits both.
     *
     * @return void
     */
